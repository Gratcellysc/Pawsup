...
if ( ! empty( $atts['background_image'] ) && ! empty( $atts['background_image']['data']['icon'] ) ) {
	$bg_image = $atts['background_image']['data']['icon'];
}
...
?>
<section class="fw-main-row <?php echo esc_attr( $section_extra_classes ) ?>" <?php if ( $bg_image ) { echo 'style="background-image:url(' . esc_url( $bg_image ) . ');"'; } ?><?php echo ( !empty( $section_id )  ) ? ' id="' . esc_attr( $section_id ) . '"' : '' ;?>>
	<h3 class="hidden"><?php echo esc_attr($section_name); ?></h3>
		<?php
			...
			?>
				...
				<div class="image_cover <?php echo ( ! empty( $atts['side_media_position'] ) ) ? esc_attr( $atts['side_media_position'] ) : '' ; ?>" style="background-image:url('<?php echo esc_url($atts['side_media_image']['url'] )?>')">
					<?php if ( $link ): ?>
